...
[GeneratedComClass]
public sealed partial class CpuCoreViewModelAdapter : IAvnRustVmSink, IAvnRustVmSink2, IAvnRustVmSink3, IRustVmStringSnapshotSink, IRustVmModelSnapshotSink, IRustVmBatchTarget, IRustVmTableSelectionBatchTarget, INotifyPropertyChanged, INotifyDataErrorInfo, IDisposable
{
    ...
    private double _usage = 0D;

    /// <summary>Creates an adapter that dispatches and posts through <see cref="Dispatcher.UIThread"/>.</summary>
    public CpuCoreViewModelAdapter(IAvnRustViewModel model) : this(model, null, null) { }
    ...
    /// </summary>
    public CpuCoreViewModelAdapter(IAvnRustViewModel model, Action<Action>? dispatch) : this(model, dispatch, null) { }
    ...
    /// batch submission.
    /// </summary>
    public CpuCoreViewModelAdapter(IAvnRustViewModel model, Action<Action>? dispatch, Action<Action>? post)
    {
        _model = model;
    ...
    /// <summary>
    /// The declared display projection (<c>Label</c>). Presentation and UI
    ...
    public int SetDouble(int propertyId, double value)
    {
        ...
        return propertyId switch
        {
            2 => Apply(() => { var converted = value; _inboundWrites.CommitPublication(propertyId, inbound); if (!Equals(_usage, converted)) SetField(ref _usage, converted, nameof(Usage)); }),
            _ => unchecked((int)0x80070057),
        };
    }
    ...
    public int SetPropertyError(int propertyId, string? message) => propertyId switch
    {
        ...
        2 => Apply(() => SetError(nameof(Usage), message)),
        _ => unchecked((int)0x80070057),
    };
    ...
    bool IRustVmBatchTarget.TryGetProperty(int propertyId, out RustVmBatchProperty property)
    {
        property = propertyId switch
        {
            ...
            2 => new RustVmBatchProperty(nameof(Usage), RustVmValueWireKind.Double, false, false),
            _ => default,
        };
    ...
    bool IRustVmBatchTarget.CommitProperty(int propertyId, in RustVmBatchValue value, out IDisposable? replaced)
    {
        ...
        switch (propertyId)
        {
            ...
            case 2:
            {
                ...
                if (Equals(_usage, next)) return false;
                _usage = next;
                return true;
            }
